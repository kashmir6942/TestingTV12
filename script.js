// Real channel data from the provided list
const channelsData = [
        ...
        channelElement.innerHTML = `
            ...
            <div class="channel-info-item">
                <div class="channel-name">
                    ${channel.name}
                    <span class="channel-drm">[DRM]</span>
                </div>
...
async function testAllChannels() {
    ...
    testAllBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Testing...';
    ...
    testAllBtn.innerHTML = '<i class="fas fa-check-circle"></i> Test All Channels';
}
...
async function selectChannel(channel) {
    ...
    try {
        ...
        alert(`Failed to load ${channel.name}. The stream may be offline or require different credentials.`);
...
function updateChannelCount() {
    ...
    const offlineCount = filteredChannels.filter(ch => ch.status === 'offline').length;
    const unknownCount = filteredChannels.filter(ch => ch.status === 'unknown').length;
    
    channelCountElement.innerHTML = `
        ${filteredChannels.length} channels 
        <span style="color: #2ed573;">●${onlineCount}</span> 
        <span style="color: #ff4757;">●${offlineCount}</span> 
        <span style="color: #6c757d;">●${unknownCount}</span>
    `;
}
